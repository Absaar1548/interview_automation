...
**Headers**:
- `X-Interview-Id` (Required for all endpoints except `/session/init`)
...
**POST /session/start**
- **State**: `RESUME_PARSED` or `READY` (transition → `READY` → `IN_PROGRESS`)
...
**Endpoint**: `WS /api/v1/proctoring/ws`
